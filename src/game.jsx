...
function game() {
    ...
    const addNumber = (prevowner, row, col) => {
      ...
      if (particle !== 0) {
        ...
        particle === 4 ? footnewala(`${row}`, `${col}`) : ""
  
        let owner = grid.getAttribute("owner");
  
        owner === "1" ? grid.innerHTML = `<div>${particle}R</div>` : ""
        owner === "2" ? grid.innerHTML = `<div>${particle}B</div>` : ""
  
        particle < 4 ? grid.setAttribute("particle", particle) : ""
  
        // changeRecords(`${owner}`, "plus")
        ...
        grid.innerHTML = `<div>1${prevowner === "1" ? "R" : "B"}</div>`
      }
    }
    ...
    const inputHandel = (row, col) => {
      ...
      if (player === "true" && (owner === "null" || owner === "1") && particle < 4) {
        ...
        if (particle >= 4) {
          ...
          displayParticle(row, col, particle, player)
        }
    ...
    const displayParticle = (row, col, value, owner) => {
      const grid = document.getElementById(`${row}${col}`)
      if (owner === "true") {
        grid.innerText = `${value}R`
      } else {
        grid.innerText = `${value}B`
      }
    }
